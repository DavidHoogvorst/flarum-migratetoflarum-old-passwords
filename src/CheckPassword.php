...
class CheckPassword
{
    ...
    {
        if (is_null($user->smf_old_password) || empty($password)) {
            return null;
        }
        ...
        $oldPassword = array('type' => 'bcrypt', 'password' => $user->smf_old_password, 'salt-before' => strtolower($user->username));
        if (substr($oldPassword['password'], 0, 4) != '$2y$') {
          $oldPassword['type'] = 'sha1';
        }
        $checker = $this->oldHasherFactory->createHasher($oldPassword['type']);
